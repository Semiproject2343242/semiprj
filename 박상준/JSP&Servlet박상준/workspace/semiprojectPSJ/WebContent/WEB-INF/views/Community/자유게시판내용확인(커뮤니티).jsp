    ...
    <section>
      ...
      <div id="main_section">
      	<form action="fmodifyForm.cm" id="detailForm" name="detailForm">
            ...
            <h4> 첨부파일 (<%= fileList.size() %>) </h4> <!-- 참고자료 링크를 어떻게 넣어야할지..-->
            <%if(fileList.isEmpty()) { %>
              	첨부파일 없음.
            <% } else { %>
           ...
           <div align="right">
				...
				function deleteBoard(){
					...
					console.log(wno);
				    if(result){
				    	location.href="<%= request.getContextPath() %>/fdelete.cm?no="+num;
				    }
				    else{
				        alert('취소하셨습니다.');
				    }
	              	...
	              	<text class="text4">
						...
						<script>
							function deleteReply(replyNo){
							    ...
							    if(result){
							    	location.href="<%= request.getContextPath() %>/replyDelete.re?replyNo="+replyNo+"&boardNo="+boardNo+"&bName=자유";
							    } else{
							        alert('취소하셨습니다.');
							    }
							}
