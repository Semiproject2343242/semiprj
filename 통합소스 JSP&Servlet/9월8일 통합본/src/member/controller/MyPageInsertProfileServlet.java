...
public class MyPageInsertProfileServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public MyPageInsertProfileServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = memberService.insertProfile(profile, loginMemberNo);
			...
			if(result > 0) {
				...
				request.setAttribute("msg", "프로필 등록에 실패했습니다.");
